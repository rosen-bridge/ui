...
- Send Rosen lock transactions through Shake Wallet
...
// Send Rosen lock transaction
const tx = await wallet.sendRosenBridgeData({
  receiver: lockAddress,
  amount,
  data,
});
```
...
- **Transaction Type**: HNS lock output plus Rosen metadata encoded in data outputs
- **Metadata**: Bridge metadata is generated by the Rosen Bridge network layer
