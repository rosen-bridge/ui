...
export class BitcoinRunesCalculator extends AbstractCalculator {
  ...
  totalRawSupply = async (token: RosenChainToken): Promise<bigint> => {
    ...
    if (tokenDetail) {
      this.logger.debug(`Total supply of token [${token.tokenId}] is [${tokenDetail.supply}]`);
  ...
  totalRawBalance = async (token: RosenChainToken): Promise<bigint> => {
    ...
    for (const address of this.addresses) {
      ...
      const addressTokenBalance = response.data.data ? BigInt(response.data.data.amount) : 0n;
  ...
  getRawLockedAmountsPerAddress = async (token: RosenChainToken) => {
    ...
    const balances = zipWith(this.addresses, tokenBalances, (address, amount) => ({
      address,
      amount,
    })).filter((amountPerAddress) => amountPerAddress.amount);

    for (const { address, amount } of balances) {
      this.logger.debug(`balance of token [${token.name}] for address [${address}] is [${amount}]`);
    }
