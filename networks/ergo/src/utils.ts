 ...
 * @param height
 * @param tokenId
 * @param wrappedAmount THIS IS A UNWRAPPED-VALUE
 * @param toChain
 * @param toAddress
 * @param fromAddress
 * @param bridgeFee
...
export const createLockBox = (
   ...
   */
  const boxErgValue = tokenId === 'erg' ? wrappedAmount : minBoxValue;
  ...
  if (tokenId !== 'erg') {
    lockBox.add_token(
      ...
      wasm.TokenAmount.from_i64(wasm.I64.from_str(wrappedAmount.toString()))
    );
  }
