...
import type { FiroUtxo, BlockCypherAddress } from './types';
...
export const getAddressUtxos = async (
  ...
  const GET_ADDRESS = `${blockcypherUrl}/v1/firo/main/addrs/${address}?unspentOnly=true&limit=500`;
...
export const getTxHex = async (txId: string): Promise<string> => {
  const blockcypherUrl = `${process.env.FIRO_BLOCKCYPHER_API}`;
  const GET_TX = `${blockcypherUrl}/v1/firo/main/txs/${txId}?includeHex=true`;
  const res = await Axios.get<{ hex: string }>(GET_TX);
  return res.data.hex;
};

/**
 * gets address Firo balance from BlockCypher
 * @param address
 * @returns this is a UNWRAPPED-VALUE amount
 */
export const getAddressBalance = async (address: string): Promise<bigint> => {
  const blockcypherUrl = `${process.env.FIRO_BLOCKCYPHER_API}`;
  const GET_ADDRESS = `${blockcypherUrl}/v1/firo/main/addrs/${address}`;
  const res = await Axios.get<BlockCypherAddress>(GET_ADDRESS);
...
export const getFeeRatio = async (): Promise<number> => {
  const blockcypherUrl = `${process.env.FIRO_BLOCKCYPHER_API}`;
  const GET_CHAIN = `${blockcypherUrl}/v1/firo/main`;
...
export const submitTransaction = async (
  ...
  const blockcypherUrl = `${process.env.FIRO_BLOCKCYPHER_API}`;
  const POST_TX = `${blockcypherUrl}/v1/firo/main/txs/push`;
...
export const getHeight = async (): Promise<number> => {
  const response = await fetch(
    `${process.env.FIRO_BLOCKCYPHER_API}/v1/firo/main`,
  );
